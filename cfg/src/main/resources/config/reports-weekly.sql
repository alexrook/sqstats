...
select week,/* неделя */
    ...
    from vr_reportsbase group by week;

---xml version 
drop view if exists vr_xml_week_sums;
create or replace view vr_xml_week_sums
as
select a.*,xmlforest(xmlforest(a.week,a.duration,a.bytes,a.conn_count) as row)
as row from vr_week_sums a;
...
(select week,/* дата */
    ...
    from vr_reportsbase
    group by week,client_host,sitegroup) as a,
    clienthost b
...
select  week,/* дата */
    ...
    from vr_reportsbase group by week,sitegroup;
