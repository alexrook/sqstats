...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:variable name="date" select="substring-before(report/resultset/column/row/week,'T')"/>
        <html lang="ru">
            <head>
                <meta charset="utf-8"/>
            ...
            <body>
                <ol class="breadcrumb">
                    ...
                    <li>
                        <a href="week_sums">Итоги по неделям</a>
                    </li>
                    <li>
                        <a>
                            <xsl:attribute name="title">
                                <xsl:value-of select="concat('итоги за неделю начиная с ',$date,' по клиентам')"/>
                            </xsl:attribute>
                            <xsl:attribute name="href">
                                <xsl:value-of
                                    select="concat('week_sums_client?week=',$date)"/>
                            </xsl:attribute>
                            Итоги за неделю начиная с <xsl:value-of select="$date"/>
                        </a>
